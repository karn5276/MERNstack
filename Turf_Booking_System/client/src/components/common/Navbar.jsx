        ...
        <div>
            {shownavbar && <div className='flex h-15 justify-center items-center border-b-[1px] border-b-white-100'>
                <div className='flex w-11/12 max-w-Content justify-between items-center'>
                    {/* logo image  */}
                    <Link to='/login'>
                        <img src={logo} width={160} alt="Turf" height={42}></img>
                    </Link>
                    ...
                    <div className={`flex md:hidden  relative gap- flex-row ${token !== null && user?.accountType !== "Owner" ? " -left-12" : ""}`}>
                        <GiHamburgerMenu className={`w-16 h-8 fill-richblack-25 absolute -bottom-4 left-10 `} onClick={shownav} />
                        <div ref={overlay} className=' fixed top-0 bottom-0 left-0 right-0 z-30 bg w-[100vw] hidden h-[100vh] overflow-y-hidden bg-[rgba(0,0,0,0.2)] ' onClick={shownav}></div>
